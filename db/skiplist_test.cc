...
TEST_CASE("SkipTest Empty", "[SkipTest]") {
  Arena arena;
  Comparator cmp;
...
TEST_CASE("SkipTest InsertAndLookup", "[SkipTest]") {
  const int N = 2000;
  const int R = 5000;
  Random rnd(1000);
  std::set<Key> keys;
  ...
  void ReadStep(Random* rnd) {
    ...
    while (true) {
      ...
      if (!iter.Valid()) {
        ...
        current = iter.key();
        // Catch2: 使用 INFO 代替 operator<<，并使用 REQUIRE
        INFO("Current key: " << current);
        REQUIRE(IsValidKey(current));
      }

      INFO("should not go backwards. Pos: " << pos << ", Current: " << current);
      REQUIRE(pos <= current);
      ...
      while (pos < current) {
        ...
        // <*,0,*> is missing.
        {
          // Catch2 INFO is scoped.
          INFO("key: " << key(pos) << "; gen: " << gen(pos)
...
// scaffolding.
TEST_CASE("SkipTest ConcurrentWithoutThreads", "[SkipTest]") {
  ConcurrentTest test;
...
TEST_CASE("SkipTest Concurrent2", "[SkipTest][Concurrent]") {
  RunConcurrent(2);
}
TEST_CASE("SkipTest Concurrent3", "[SkipTest][Concurrent]") {
  RunConcurrent(3);
}
TEST_CASE("SkipTest Concurrent4", "[SkipTest][Concurrent]") {
  RunConcurrent(4);
}
TEST_CASE("SkipTest Concurrent5", "[SkipTest][Concurrent]") {
  RunConcurrent(5);
}
